...
- Before building generic matching, parsing, address handling, or synchronization primitives, first evaluate mature third-party libraries and prefer a dependency over custom code when the library is actively maintained, typed or easy to wrap, and fits the project constraints. Custom code should be limited to project-specific glue, policy, and fallback behavior.
...
- Restaurant/place/address fixtures must come from recorded real data or explicit user-provided examples; do not invent restaurant names, place candidates, or addresses for tests.
